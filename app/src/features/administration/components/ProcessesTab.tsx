...
import { AlertTriangle, Globe, MoreHorizontal, Pencil, Plus, Search, Trash2, Workflow } from 'lucide-react'
...
export function ProcessesTab(): React.ReactElement {
  ...
  const { data, isLoading, error, isFetching } = useProcesses(
    page,
    ITEMS_PER_PAGE,
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-muted-foreground">
          {t(
            'administration.processes.description',
            'Manage processes and their configurations.'
          )}
        </p>
        <button
        ...
        >
          <Plus size={16} />
          {t('administration.processes.create', 'Create Process')}
        </button>
      </div>
      ...
      <div className="group relative w-full md:max-w-xs">
        ...
        <input
          type="text"
          placeholder={t('administration.processes.searchPlaceholder', 'Search processes...')}
          value={searchQuery}
      ...
      <div className="rounded-sm border">
        ...
        {error && !isLoading && (
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {t('administration.processes.loadError', 'Failed to load processes')}
            </p>
          </div>
        )}
        ...
        {!isLoading && !error && processes.length === 0 && (
          <div className="flex flex-col items-center justify-center p-12 text-center">
            <Workflow size={32} className="mb-3 text-muted-foreground/50" />
            <p className="text-sm text-muted-foreground">
              {debouncedQuery.length >= 3
                ? t('administration.processes.noResults', 'No processes match your search')
                : t('administration.processes.empty', 'No processes found')}
            </p>
          </div>
        )}
            ...
            <thead>
              <tr className="border-b">
                <th className={TH_CLASS}>
                  {t('administration.processes.columns.code', 'Code')}
                </th>
                <th className={TH_CLASS}>
                  {t('administration.processes.columns.processType', 'Process Type')}
                </th>
                <th className={TH_CLASS}>
                  {t('administration.processes.columns.name', 'Name')}
                </th>
                <th className={TH_CLASS}>
                  {t('administration.processes.columns.templates', 'Templates')}
                </th>
                <th className={`${TH_CLASS} w-12`}>
                  {t('administration.processes.columns.actions', 'Actions')}
                </th>
              </tr>
            </thead>
                ...
                <tr key={process.id} className="border-b last:border-0 hover:bg-muted/50">
                  ...
                  <td className="p-4 font-medium">
                    <div className="flex items-center gap-2">
                      {getLocalizedName(process.name, i18n.language)}
                      {process.isGlobal && (
                        <Badge variant="secondary" className="gap-1 text-xs">
                          <Globe size={10} />
                          {t('administration.processes.global', 'Global')}
                        </Badge>
                      )}
                    </div>
                  </td>
                  <td className="p-4 font-mono text-sm text-muted-foreground">
                    {process.templatesCount ?? 0}
                  </td>
                  <td className="p-4">
                    {!process.isGlobal ? (
                      <DropdownMenu>
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t('administration.processes.readOnly', 'Read-only')}
                      </span>
                    )}
                  </td>
                </tr>
