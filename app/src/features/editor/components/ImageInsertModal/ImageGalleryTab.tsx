...
export function ImageGalleryTab({ onSelect, currentSelectionKey }: ImageGalleryTabProps) {
  ...
  const [selectedKey, setSelectedKey] = useState<string | null>(currentSelectionKey ?? null)
  const [resolvedURLs, setResolvedURLs] = useState<Record<string, string>>({})
  const [error, setError] = useState<string | null>(null)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const resolvedURLsRef = useRef<Record<string, string>>({})
  const previousAssetKeysRef = useRef<Set<string>>(new Set())
  ...
  useEffect(() => {
    setSelectedKey(currentSelectionKey ?? null)
  }, [currentSelectionKey])
  ...
  useEffect(() => {
    ...
    const removedKeys = Array.from(previousAssetKeysRef.current).filter((key) => !currentKeys.has(key))
    ...
    previousAssetKeysRef.current = currentKeys

    let cancelled = false

    for (const asset of assets) {
      if (resolvedURLsRef.current[asset.key]) continue
      ...
      galleryApi
        ...
        .then((url) => {
          ...
          setResolvedURLs((prev) => {
            if (prev[asset.key]) {
              if (url.startsWith('blob:') && prev[asset.key] !== url) {
                URL.revokeObjectURL(url)
              }
              return prev
            }

            const next = { ...prev, [asset.key]: url }
            resolvedURLsRef.current = next
            return next
          })
        })
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setAssets((prev) => [asset, ...prev.filter((item) => item.key !== asset.key)])
        setTotal((prev) => (assets.some((item) => item.key === asset.key) ? prev : prev + 1))
        setSelectedKey(asset.key)
      ...
      <div className="flex-1">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
            ...
            <p className="font-mono text-xs">{debouncedQuery ? 'Sin resultados' : 'La galería está vacía'}</p>
          </div>
        ) : (
            ...
            {assets.map((asset) => (
              ...
              >
                {resolvedURLs[asset.key] ? (
                  <img src={resolvedURLs[asset.key]} alt={asset.filename} className="h-full w-full object-cover" />
                ) : (
