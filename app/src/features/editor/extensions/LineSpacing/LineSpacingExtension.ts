...
function renderLineSpacingAttributes(lineSpacing?: string | null) {
  if (!lineSpacing || normalizeLineSpacingPreset(lineSpacing) === DEFAULT_LINE_SPACING) {
    return {}
  }
    ...
    return [
      {
        ...
        attributes: {
          lineSpacing: {
            ...
            renderHTML: (attributes) => renderLineSpacingAttributes(attributes.lineSpacing),
          },
        },
      },
    ]
    ...
    return {
      setLineSpacing:
        ...
        ({ commands }) => {
          ...
          return this.options.types
            .map((type) => commands.updateAttributes(type, { lineSpacing: preset }))
            .some((response) => response)
        },

      unsetLineSpacing:
        () =>
        ({ commands }) => this.options.types
          .map((type) => commands.resetAttributes(type, 'lineSpacing'))
          .some((response) => response),
    }
  },
