...
// anchorRegex validates signer anchor format (__sig_rolename__).
var anchorRegex = regexp.MustCompile(`^__sig_[a-z][a-z0-9_]*__$`)
...
// TemplateVersionSignerRole defines a signature role for a template version and its PDF anchor.
...
// NewTemplateVersionSignerRole creates a new signer role for a template version.
func NewTemplateVersionSignerRole(templateVersionID, roleName, anchorString string, order int) *TemplateVersionSignerRole {
...
// Validate checks if the signer role data is valid.
func (tvsr *TemplateVersionSignerRole) Validate() error {
	if tvsr.TemplateVersionID == "" {
		return ErrRequiredField
	}
	if tvsr.RoleName == "" {
		return ErrRequiredField
	}
	if len(tvsr.RoleName) > 100 {
		return ErrFieldTooLong
	}
	if tvsr.AnchorString == "" {
		return ErrRequiredField
	}
	if len(tvsr.AnchorString) > 100 {
		return ErrFieldTooLong
	}
	if !anchorRegex.MatchString(tvsr.AnchorString) {
		return ErrInvalidSignerRole
	}
	if tvsr.SignerOrder < 1 {
		return ErrInvalidSignerRole
	}
	return nil
}
...
type TemplateVersionWithDetails struct {
	...
	SignerRoles []*TemplateVersionSignerRole       `json:"signerRoles,omitempty"`
}
