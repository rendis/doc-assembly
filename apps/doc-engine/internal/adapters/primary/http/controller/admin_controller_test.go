...
// TestAdminController_SystemWorkspaces tests system workspace endpoints.
func TestAdminController_SystemWorkspaces(t *testing.T) {
	pool := testhelper.GetTestPool(t)
	ts := testhelper.NewTestServer(t, pool)
	client := testhelper.NewHTTPClient(t, ts.URL())

	superAdmin := testhelper.CreateTestUser(t, pool,
		"superadmin-ws@test.com", "Super Admin",
		testhelper.Ptr(entity.SystemRoleSuperAdmin))
	defer testhelper.CleanupUser(t, pool, superAdmin.ID)

	platformAdmin := testhelper.CreateTestUser(t, pool,
		"platformadmin-ws@test.com", "Platform Admin",
		testhelper.Ptr(entity.SystemRolePlatformAdmin))
	defer testhelper.CleanupUser(t, pool, platformAdmin.ID)

	t.Run("CreateGlobal", func(t *testing.T) {
		t.Run("success", func(t *testing.T) {
			req := dto.CreateSystemWorkspaceRequest{
				Name: "Global System Workspace",
			}

			resp, body := client.WithAuth(superAdmin.BearerHeader).
				POST("/api/v1/system/workspaces", req)
			...
			if resp.StatusCode == http.StatusCreated {
				...
				assert.Equal(t, "SYSTEM", workspace.Type)
				assert.Nil(t, workspace.TenantID)

				defer testhelper.CleanupWorkspace(t, pool, workspace.ID)
			} else {
				// Already exists from previous test or setup
				assert.Equal(t, http.StatusConflict, resp.StatusCode)
			}
		})
		...
		t.Run("forbidden PLATFORM_ADMIN", func(t *testing.T) {
			req := dto.CreateSystemWorkspaceRequest{
				Name: "Forbidden Global Workspace",
			}
	...
	t.Run("CreateForTenant", func(t *testing.T) {
		t.Run("success", func(t *testing.T) {
			tenantID := testhelper.CreateTestTenant(t, pool, "Tenant For Workspace", "TWS001")
			defer testhelper.CleanupTenant(t, pool, tenantID)

			req := dto.CreateSystemWorkspaceRequest{
				Name: "Tenant System Workspace",
			}

			resp, body := client.WithAuth(superAdmin.BearerHeader).
				POST("/api/v1/system/tenants/"+tenantID+"/system-workspace", req)
			...
			assert.NotEmpty(t, workspace.ID)
			assert.Equal(t, "SYSTEM", workspace.Type)
			if assert.NotNil(t, workspace.TenantID) {
				assert.Equal(t, tenantID, *workspace.TenantID)
			}
		})

		t.Run("tenant not found", func(t *testing.T) {
			req := dto.CreateSystemWorkspaceRequest{
				Name: "No Tenant Workspace",
			}

			resp, _ := client.WithAuth(superAdmin.BearerHeader).
				POST("/api/v1/system/tenants/00000000-0000-0000-0000-000000000000/system-workspace", req)

			assert.Equal(t, http.StatusNotFound, resp.StatusCode)
		})

		t.Run("conflict already exists", func(t *testing.T) {
			tenantID := testhelper.CreateTestTenant(t, pool, "Conflict Tenant", "CON001")
			defer testhelper.CleanupTenant(t, pool, tenantID)

			// Create first workspace
			wsID := testhelper.CreateTestWorkspace(t, pool, &tenantID, "First System", entity.WorkspaceTypeSystem)
			defer testhelper.CleanupWorkspace(t, pool, wsID)

			// Try to create second
			req := dto.CreateSystemWorkspaceRequest{
				Name: "Second System Workspace",
			}
