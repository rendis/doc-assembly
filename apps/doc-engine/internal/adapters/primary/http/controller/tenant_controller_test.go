...
// TestTenantController_ListMyTenantWorkspaces tests the GET /tenant/my-workspaces endpoint.
func TestTenantController_ListMyTenantWorkspaces(t *testing.T) {
	pool := testhelper.GetTestPool(t)
	ts := testhelper.NewTestServer(t, pool)
	client := testhelper.NewHTTPClient(t, ts.URL())

	// Create test tenant
	tenantID := testhelper.CreateTestTenant(t, pool, "My Workspaces Tenant", "MWNT01")
	defer testhelper.CleanupTenant(t, pool, tenantID)

	// Create owner
	owner := testhelper.CreateTestUser(t, pool, "owner-myws@test.com", "Owner User", nil)
	defer testhelper.CleanupUser(t, pool, owner.ID)
	testhelper.CreateTestTenantMember(t, pool, tenantID, owner.ID, entity.TenantRoleOwner, nil)

	// Create workspace and assign member
	wsID := testhelper.CreateTestWorkspace(t, pool, &tenantID, "My Workspace", entity.WorkspaceTypeClient)
	defer testhelper.CleanupWorkspace(t, pool, wsID)
	testhelper.CreateTestWorkspaceMember(t, pool, wsID, owner.ID, entity.WorkspaceRoleOwner, nil)

	t.Run("success with assigned workspace", func(t *testing.T) {
		resp, body := client.
			WithAuth(owner.BearerHeader).
			WithTenantID(tenantID).
			GET("/api/v1/tenant/my-workspaces")

		assert.Equal(t, http.StatusOK, resp.StatusCode)

		var listResp dto.ListResponse[dto.WorkspaceWithRoleResponse]
		err := json.Unmarshal(body, &listResp)
		require.NoError(t, err)
	...
	t.Run("success with no workspace assignments", func(t *testing.T) {
		// Create user with tenant membership but no workspace assignments
		noWsUser := testhelper.CreateTestUser(t, pool, "nows@test.com", "No WS User", nil)
		defer testhelper.CleanupUser(t, pool, noWsUser.ID)
		testhelper.CreateTestTenantMember(t, pool, tenantID, noWsUser.ID, entity.TenantRoleAdmin, nil)

		resp, body := client.
			WithAuth(noWsUser.BearerHeader).
			WithTenantID(tenantID).
			GET("/api/v1/tenant/my-workspaces")
	...
	t.Run("forbidden for non-member", func(t *testing.T) {
		nonMember := testhelper.CreateTestUser(t, pool, "nonmember-myws@test.com", "Non Member", nil)
		defer testhelper.CleanupUser(t, pool, nonMember.ID)
