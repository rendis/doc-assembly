...
const (
	queryCreate = `
		INSERT INTO execution.document_recipients (
			...
			signer_recipient_id, status, signed_at, created_at
	...
	queryFindByDocumentIDWithRoles = `
		SELECT dr.id, dr.document_id, dr.template_version_role_id, dr.name, dr.email,
			   dr.signer_recipient_id, dr.status, dr.signed_at, dr.created_at, dr.updated_at,
	...
	queryUpdate = `
		...
		SET name = $2, email = $3, signer_recipient_id = $4, status = $5,
			signed_at = $6, updated_at = $7
		WHERE id = $1
	`
