...
export function FormulaSummary({ schema, className }: FormulaSummaryProps) {
  ...
  const summary = useMemo(() => {
    const generateRuleSummary = (rule: LogicRule): string => {
      ...
      if (['empty', 'not_empty', 'is_true', 'is_false'].includes(rule.operator)) {
  ...
  return (
    <motion.div
      className={cn('flex items-start gap-3 p-3 rounded-lg border bg-muted/30', className)}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <motion.code
          key={summary}
          className={cn('text-sm font-mono break-all', isEmpty ? 'text-muted-foreground italic' : 'text-foreground')}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
