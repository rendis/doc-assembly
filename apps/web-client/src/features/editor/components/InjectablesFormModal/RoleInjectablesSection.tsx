    ...
    <Collapsible open={!isCollapsed} onOpenChange={(newOpen) => setIsCollapsed(!newOpen)}>
      ...
      <CollapsibleContent>
        <div className="space-y-4 bg-muted/30 p-3 rounded-md border border-muted">
          {Array.from(roleGroups.entries()).map(([roleLabel, injectables], index) => (
            <div key={roleLabel}>
              <h3 className="text-sm font-semibold text-foreground mb-3">{roleLabel}</h3>
              ...
              {/* Separador entre roles (excepto el último) */}
