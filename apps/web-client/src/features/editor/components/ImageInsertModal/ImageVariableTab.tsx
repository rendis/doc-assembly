...
export function ImageVariableTab({ onSelect, currentSelection, hasUrlSelection }: ImageVariableTabProps) {
  ...
  useEffect(() => {
    if (currentSelection && imageVariables.length > 0 && !previewUrl) {
      ...
      if (variable) {
        ...
        // eslint-disable-next-line react-hooks/set-state-in-effect -- Intentional: initialize preview on first load
        setPreviewUrl(url)
      }
    }
  ...
  useEffect(() => {
    if (hasUrlSelection) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- Intentional reset when user selects URL
      setSelectedId(null)
      setPreviewUrl(null)
    }
  }, [hasUrlSelection])
