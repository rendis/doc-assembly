...
interface UsePreviewPDFReturn {
  ...
  error: Error | null;
  pdfBlob: Blob | null;
  generatePreview: (injectables: Record<string, any>) => Promise<void>;
  clearError: () => void;
  clearPDF: () => void;
}
 ...
 */
export function usePreviewPDF({ templateId, versionId }: UsePreviewPDFOptions): UsePreviewPDFReturn {
  const [isGenerating, setIsGenerating] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [pdfBlob, setPdfBlob] = useState<Blob | null>(null);

  const generatePreview = useCallback(async (injectables: Record<string, any>) => {
    setIsGenerating(true);
    setError(null);

    try {
      const blob = await previewApi.generate(templateId, versionId, { injectables });
      setPdfBlob(blob);
    } catch (err) {
