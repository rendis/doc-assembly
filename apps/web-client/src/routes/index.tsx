...
import { Folder, FileText, Settings, Search, Loader2, ChevronLeft, ChevronRight } from 'lucide-react'
    ...
    <div className="container mx-auto max-w-6xl py-8 text-foreground transition-colors min-h-screen">
      ...
      <div className={cn(
        ...
        {isInitialLoading ? (
            ...
            {[...Array(8)].map((_, i) => (
                <div key={i} className="h-48 animate-pulse rounded-xl bg-card border border-muted/50" />
            ))}
            </div>
        ) : workspaces.length === 0 ? (
            ...
            <p className="text-muted-foreground">{search ? 'No results found.' : t('workspace.no_workspaces')}</p>
            </div>
        ) : (
            <>
                ...
                {workspaces.map((ws) => (
                    ...
                    <div className="mb-4 flex items-start justify-between">
                        ...
                        {ws.type === 'SYSTEM' ? <Settings className="h-5 w-5" /> : <Folder className="h-5 w-5" />}
                        </div>
                    ...
                    <p className="text-xs text-muted-foreground line-clamp-2 min-h-[2rem]">
                        {ws.type === 'SYSTEM' ? t('workspace.type_system') : t('workspace.type_client')}
                    </p>
