...
interface AuthState {
  ...
  clearAuth: (showToast?: boolean) => void
  ...
  persist(
    (set, get) => ({
      ...
      clearAuth: (showToast = true) => {
        ...
        // Show toast only if session expired (not on initial load or explicit suppression)
        if (wasAuthenticated && showToast) {
          toast({
            variant: 'destructive',
